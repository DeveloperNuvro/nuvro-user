...
export default function DashboardLayout() {
  ...
  useEffect(() => {
    if (!userExists || oneSignalInitialized.current) {
      return;
    }
...
const getBreadcrumb = () => {
  ...
  return segments.map((segment, index) => {
    ...
    return (
      <span className= { cn("text-[#D4D8DE] dark:text-[#A3ABB8] text-[16px]", isLast && "font-500 text-[16px] text-[#101214] dark:text-[#FFFFFF]")
} key = { index } >
